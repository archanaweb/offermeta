...
import { addManagerPostback } from "../../../../Redux/ManagerSlice";
import { fetchPublisherList } from "../../../../Redux/PublisherSlice";

const ManagerPostbackAdd = ()=>{
...
const managerrUser =  JSON.parse(localStorage.getItem('userData'))

  useEffect(()=>{
    dispatch(fetchOfferList({partners_Id: adminPartner, currentPage: 1}))
    dispatch(fetchPublisherList(adminPartner))
},[])
  const navigate = useNavigate();

    const [formData, setFormData] = useState({})
    const handleChange = (e) => {
        setFormData({ ...formData, [e.target.name]: e.target.value, partners_Id: managerrUser?.partners_Id,managerId: managerrUser?.managerId });
      };
    
      const handleSubmit = async (e) => {
        e.preventDefault();
        const postbackResponse = await dispatch(addManagerPostback(formData));
      ...
      useEffect(()=> {
        dispatch(fetchEventValue({partners_Id:adminPartner, offerId: formData.offerId}))
    },[formData.offerId])
    return (
        <>
      ...
      onChange={handleChange}>
        ...
        {PublisherList?.map((item)=> <option value={item.publisherId}>(ID: {item.publisherId}) {item.firstName} {item.lastName}</option>)}
      </select>
    </div>
    <div className="form-group col-lg-6">
...
export default ManagerPostbackAdd;