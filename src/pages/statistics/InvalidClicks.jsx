...
const InvalidClicks = () => {
    ...
    const [invalidClick, setInvalidClick] = useState();
  ...
  const fetchData = async(apiEndpoint)=> {
    ...
    try{
      ...
      setInvalidClick(res?.responseResult)
      setLoding(false)
  ...
  useEffect(()=> {
    fetchData(`conversion/invalidClick?partners_Id=${loggedIn?._id}&page=${getCurrentPage}`)
  ...
  useEffect(()=> {
    {startDate ? fetchData(`conversion/invalidClick?partners_Id=${loggedIn?._id}&page=${getCurrentPage}&startDate=${startDate}&endDate=${endDate}`) : fetchData(`conversion/invalidClick?partners_Id=${loggedIn?._id}&page=${getCurrentPage}`)}
  },[getCurrentPage])
  
  const handleClickDate = ()=> {
    fetchData(`conversion/invalidClick?partners_Id=${loggedIn?._id}&page=${getCurrentPage}&startDate=${startDate}&endDate=${endDate}`)
  }
  useEffect(()=> {
  ...
  return (
        <div className='page_sec pt-3'>
            ...
            <TablePagination totalPage={totalPage} path='/conversion/invalidclicks' />        
            </div>
          </div>
        </div>
    );
