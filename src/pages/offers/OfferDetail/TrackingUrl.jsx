...
const TrackingUrl = ({offerId, trackingPublisherId}) => {
  ...
  const mainPartner = partnerId._id;
  ...
  const updateTrackingUrl = ()=> {
    ...
    if(formData.landingpage){
      url.searchParams.set('link_id', landingpage);
    }else{
  ...
  const generateTrackingUrl = (selectedId) => {
    const trackingLinkUrl = `https://go2.clicksmeta.com/tracking/click?m=${partnerId._id}&o=${data.offerId}&a=${selectedId}`;
    const mainTrackingLinkUrl = `https://click.adomobi.com/tracking/click?m=${partnerId._id}&o=${data.offerId}&a=${selectedId}`;
   
    if (selectedId) {
      if(mainPartner === 2 || mainPartner === 6){
